            ...
            results.append(
                {
                    ...
                    "description": f"{q[1].strftime('%d.%m.%Y.') if q[1] else ''}{q[2].strftime('%H:%M')} {'to' if q[2] else ''} {q[3].strftime('%H:%M')} - {f'{q[5]}; {q[6]}' if q[5] else ''}{q[7] if q[7] else ''}{f' | Kategorie: {q[10]}' if len(q) > 10 and q[10] else ''}{f' | Beschreibung: {q[9]}' if len(q) > 9 and q[9] else ''}",
                }
            )
    else:
        results.append(
